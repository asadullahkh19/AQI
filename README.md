...
**Models:** Ridge · Random Forest · LSTM (best auto-selected by RMSE)
...
### Fastest — full pipeline (one shot)
...
Opens at **http://localhost:8501**. London history + 72h forecast charts.

> First load is slow (downloads ~144MB model from cloud). Cached after — instant on repeat. Login cached per session.
>
...
Serves at **http://localhost:5000**. Endpoints: current AQI + 72h forecast JSON (London).
...
```bash
# Backfill synthetic/historical features → cloud (London)
...
| `No cloud features for london` | Run `python scripts/run_feature_pipeline.py` |
