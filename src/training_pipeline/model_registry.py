...
def save_model(model, city: str, model_name: str, metrics: dict, meta: dict) -> str:
    ...
    joblib.dump(payload, path, compress=3)   # shrink artifact (RF ~144MB->~15MB) for reliable cloud transfer
...
def _load_from_hopsworks(city: str, model_name: str | None):
    ...
    # Disk cache: skip the download if this exact version already pulled+validated.
    ...
    if cached:
        try:
            payload = joblib.load(cached[0])          # cache hit — validate it
        except (EOFError, ValueError, OSError) as e:  # truncated/corrupt cache
            logger.warning("Corrupt cached model %s (%s) — purging, re-downloading.",
                           cached[0], e)
            import shutil
    ...
    if payload is None:
        ...
        payload = joblib.load(files[0])               # validate BEFORE caching
        cache_dir.mkdir(parents=True, exist_ok=True)  # only cache a good download
        import shutil
        for f in files:
